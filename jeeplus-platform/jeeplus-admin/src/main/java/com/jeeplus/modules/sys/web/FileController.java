...
//			response.setHeader("Content-Disposition", "attachment; filename=" + file.getName());
	...
	@RequiresPermissions("user")
	@RequestMapping(value = "webupload/uploadTemplate")
	@ResponseBody
	public AjaxJson uploadTemplate( HttpServletRequest request, HttpServletResponse response,MultipartFile file) throws IllegalStateException, IOException {
		AjaxJson j = new AjaxJson();
		// 判断文件是否为空
		if (!file.isEmpty()) {
			String fileDir = FileKit.getTemplateDir();
			FileUtils.createDirectory(fileDir);
			String name = file.getOriginalFilename();
			String filePath = fileDir +  name;
			String fileUrl = FileKit.getTemplateUrl();
			File newFile = FileUtils.getAvailableFile(filePath,0);
			file.transferTo(newFile);
			j.put("id", newFile.getAbsolutePath());
			j.put("url", fileUrl+ newFile.getName());
