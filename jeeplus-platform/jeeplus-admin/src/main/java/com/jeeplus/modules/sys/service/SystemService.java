...
public class SystemService extends BaseService  {
	...
	public Page<User> findUser(Page<User> page, User user) {
		...
		// 设置分页参数
		user.setPage(page);
		List<User> data = userMapper.findList(user);
		List<User> list = Lists.newArrayList();
		if(data!=null){
			data.forEach(u->{
				User getRole = userMapper.get(u.getId());
				if(getRole!=null&&getRole.getRoleList()!=null) {
					u.setRoleList(getRole.getRoleList());
				}				
				list.add(u);
			});
		}
...
//		systemRealm.clearAllCachedAuthorizationInfo();
	...
	public void saveRole(Role role) {
		if (StringUtils.isBlank(role.getId())){
			...
			if(role.getOffice().getId() == null || StringUtils.isBlank(role.getOffice().getId())) {
				role.setOffice(roleMapper.getOfficeid(role.getId()));
			}	
			role.preUpdate();
