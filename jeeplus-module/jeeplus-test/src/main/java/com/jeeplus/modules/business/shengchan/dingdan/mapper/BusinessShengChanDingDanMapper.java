...
public interface BusinessShengChanDingDanMapper extends BaseMapper<BusinessShengChanDingDan> {
    ...
    @Update("update business_shengchan_dingdan_mingxi set status = '锁定' where id = #{mxid}")
    void recoverByMxid(@Param("mxid") String mxid);

    @Update("update business_shengchan_dingdan_mingxi set status = '锁定' where pid = #{mid}")
