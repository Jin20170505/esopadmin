...
$(document).ready(function() {
  ...
  function getIdSelections() {
	...
	function updateStatus() {
        ...
        jp.get("${ctx}/qiyewx/sp/qiYeWxSp/updateStatus", function(data){
            if(data.success){
                refresh();
                jp.toastr_success(data.msg);
            }else{
...
function goToSync(){
    jp.openSaveDialog('同步数据', "${ctx}/qiyewx/sp/qiYeWxSp/goToSync",'600px', '500px');
}
   //新增表单页面
 function add() {
...
<script type="text/template" id="qiYeWxSpChildrenTpl">//<!--
	...
	<script type="text/template" id="qiYeWxSpChild1Tpl">//<!--
				<tr>
					...
					<td>
					{{row.punchCorrectionState}}
					</td>
					<td>
					{{row.punchCorrectionTime}}
					</td>
				</tr>//-->
	</script>
