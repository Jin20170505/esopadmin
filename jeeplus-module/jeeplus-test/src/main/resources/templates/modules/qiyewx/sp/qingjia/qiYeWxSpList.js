...
function goToSync(){
    jp.openSaveDialog('同步数据', "${ctx}/qiyewx/sp/qiYeWxSp/goToSyncByType?type=1",'600px', '500px');
}
   //新增表单页面
 function add() {
  ...
  function detailFormatter(index, row) {
	  ...
	  $.get("${ctx}/qiyewx/sp/qiYeWxSp/detail?id="+row.id, function(qiYeWxSp){
		...
		for (var i=0; i<data1.length; i++){
			data1[i].dict = {};
			var time= data1[i].vacationAttendanceDateRangeDuration;
			if(time){
				var day = time / (3600*24);
                data1[i].vacationAttendanceDateRangeDuration=day+"天";
			}
...
<script type="text/template" id="qiYeWxSpChildrenTpl">//<!--
	...
	<script type="text/template" id="qiYeWxSpChild1Tpl">//<!--
				<tr>
					...
					<td>
						{{row.vacationAttendanceDateRangeDuration}}
					</td>
				</tr>//-->
	</script>
