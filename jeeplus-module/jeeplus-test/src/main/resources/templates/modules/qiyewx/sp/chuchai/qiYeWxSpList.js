...
function goToSync(){
    jp.openSaveDialog('同步数据', "${ctx}/qiyewx/sp/qiYeWxSp/goToSyncByType?type=3",'600px', '500px');
}
   //新增表单页面
 function add() {
  ...
  function detailFormatter(index, row) {
	  ...
	  $.get("${ctx}/qiyewx/sp/qiYeWxSp/detail?id="+row.id, function(qiYeWxSp){
		...
		for (var i=0; i<data1.length; i++){
            ...
            if(time){
                var day = time / (3600);
                data1[i].vacationAttendanceDateRangeDuration=day+"h";
            }
