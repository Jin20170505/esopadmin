		...
		$(document).ready(function() {
			...
			$('#search_q').keyup(function () {
				...
				to = setTimeout(function () {
					var v = $('#search_q').val();
					$('#testLeftTreejsTree').jstree(true).search(v);
				}, 250);
			});
			$('#testLeftTreejsTree').jstree({
				'core' : {
					"multiple": false,
                    "animation": 0,
                     "themes": {"icons": true, "stripes": false},
					'data' : {
						"url" : "${ctx}/test/leftright/testLeftTree/treeData",
						"dataType" : "json" 
					}
				},
					...
					"items": function (node) {
						...
						tmp.remove = {
							...
							"action": function (data) {
								var inst = jQuery.jstree.reference(data.reference),
									obj = inst.get_node(data.reference);
								jp.confirm('确认要删除左树右表吗？', function(){
									jp.loading();
									$.get("${ctx}/test/leftright/testLeftTree/delete?id="+obj.id, function(data){
										if(data.success){
											$('#testLeftTreejsTree').jstree("refresh");
											jp.success(data.msg);
										}else{
											jp.error(data.msg);
										}
			...
			"types": {
				...
				var node = $('#testLeftTreejsTree').jstree(true).get_selected(true)[0];
				var opt = {
					silent: true,
					query:{
						'p.id':node.id
				...
				$("#pId").val(node.id);
				$("#pName").val(node.text);
				$('#testRightTableTable').bootstrapTable('refresh',opt);
			}).on('loaded.jstree', function() {
				$("#testLeftTreejsTree").jstree('open_all');
			});
		});
